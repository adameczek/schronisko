...
@Tag(
        name = "Animals",
        description = "Provides data for animals"
)
public class AnimalController {
    ...
    ResponseEntity<Page<AnimalResponse>> getAnimals(
        ...
        return ResponseEntity.ok(animalService.getAnimals(pageable).map(this::convertToResponse));
    }
    ...
    ResponseEntity<AnimalResponse> createAnimal(
            @RequestBody
            @Validated
            AnimalRequest animal) throws InsufficentUserRoleException, AnimalServiceException {
        ...
        if (authenticatedUser.getRoles().stream().noneMatch(role -> Role.ADMIN.equals(role)
                                                                    || Role.MODERATOR.equals(role))) {
            throw new InsufficentUserRoleException("User is not authorized to add animals!");
        }
        
        Animal newAnimal = animalService.createAnimal(animal, authenticatedUser);
    ...
    ResponseEntity<AnimalResponse> updateAnimal(
        ...
        if (authenticatedUser.hasNoRoles(Role.ADMIN, Role.MODERATOR)) {
            throw new InsufficentUserRoleException("Uzytkownik nie ma uprawnien do tego requestu.");
        }
        return ResponseEntity.ok(convertToResponse(animalService.updateAnimal(id, request)));
    }
    ...
    ResponseEntity<Page<AnimalResponse>> findAnimals(
            @ParameterObject Pageable pageable,
            @RequestBody AnimalSearchQuery searchQuery) throws AnimalServiceException {
        Page<AnimalResponse> animals = animalService.searchForAnimals(searchQuery, pageable)
    ...
    @ExceptionHandler(
            {AnimalServiceException.class, HttpMessageNotReadableException.class}
    )
    ResponseEntity<ErrorResponse> animalServiceException(Exception e, WebRequest request) {
    ...
    ResponseEntity<SimpleResponse> deleteAnimal(
        ...
        if (authenticatedUser.hasNoRoles(Role.ADMIN)
            && animalToDelete.getShelter().getName() != authenticatedUser.getShelter().getName()) {
            throw new InsufficentUserRoleException(
