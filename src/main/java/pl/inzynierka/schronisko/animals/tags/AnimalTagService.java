...
public class AnimalTagService {
    ...
    public AnimalTag saveTag(AnimalTag animalTag) throws InsufficentUserRoleException, AnimalTagServiceException {
        ...
        try {
            ...
            e.printStackTrace();
            throw new AnimalTagServiceException("Wystąpił błąd przy zapisie nowego tag'a");
        }
    }
    
    public SimpleResponse deleteTag(String value) throws InsufficentUserRoleException {
        final User authenticatedUser = AuthenticationUtils.getAuthenticatedUser();
        if (authenticatedUser.hasNoRoles(Role.ADMIN, Role.MODERATOR)) {
